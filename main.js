...
function number_To_Words(num) {
    ...
    if (num < 10) {
        ...
        var tens_Part = tens[Math.floor(num / 10)];
        var ones_Part = num % 10;
        if (ones_Part === 0) {
            return tens_Part;
        }
        else {
            return "".concat(tens_Part, "-").concat(single_Digits[ones_Part]);
        }
    }
}
;
...
//   // Your code here
// }
function sum_Of_First_N_Numbers_loop(n) {
    var sum = 0;
    for (var i = 1; i <= n; i++) {
        sum += i;
    }
...
console.log(sum_Of_First_N_Numbers_loop(5)); // Output: 15
console.log(sum_Of_First_N_Numbers_loop(10)); // Output: 55
